...
{ /// <summary>
    ...
    /// </summary>
    internal class ZObservableDbOperation : ZDbOperation, IDisposable, IDbBehavior
    {
        ...
        public ZObservableDbOperation(string connectionString, string commandText, TraceSource traceSource, IDbProviderFactory dbProviderFactory, IDbBehavior dbBehavior)
        ...
        public ZObservableDbOperation(string connectionString, string commandText, TraceSource traceSource, params IDataParameter[] parameters)
        ...
        public void ExecuteReaderWithUpdates<T>(Action<T, ZDbOperation> processRecord)
        {
            lock (_stopLocker)
            {
                if (_disposing)
            ...
            for (var i = 0; i < delays.Count; i++)
            {
                ...
                for (var j = 0; j < retryCount; j++)
                {
                    ...
                    try
                    {
                        recordCount = ExecuteReader<T>(processRecord);
        ...
        protected virtual void SqlDependency_OnChange(SqlNotificationEventArgs e, Action<Messages_0, DbOperation> processRecord)
        {
